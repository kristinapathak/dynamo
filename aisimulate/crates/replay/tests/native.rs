...
use aisimulate_engine::{
    NativeBackend, NativeEngineConfig, NativeSglangConfig, NativeTimingModel,
    NativeTimingModelConfig,
};
use aisimulate_replay::{
    AggregatedRoundRobinPlacement, NativeReplayEngineConfig, NativeReplayRoleConfig,
    NoEngineEvents, NoReplayMetadata, PoolRoundRobinPlacement, ProviderSpec, ReplayAdapters,
    ReplayComposition, ReplayRequest, ReplayScalingDecision, ReplayScalingPolicy,
    ReplayScalingSnapshot, ReplaySpec, ReplayTopology, Replayer, TraceSimulationReport,
    WorkerPoolSpec, WorkerTopology, run_native_replay, run_native_replay_with_optional_role_timing,
    run_native_replay_with_timing,
};
use anyhow::Result;
...
fn engine_config(timing_model: NativeTimingModelConfig) -> NativeReplayEngineConfig {
    NativeReplayEngineConfig {
        dp_size: 1,
...
fn spec(config: NativeReplayEngineConfig) -> ReplaySpec {
    ReplaySpec {
        version: 1,
...
fn disaggregated_spec(
    backend: NativeBackend,
    prefill_timing: NativeTimingModelConfig,
    decode_timing: NativeTimingModelConfig,
) -> ReplaySpec {
    let mut spec = spec(NativeReplayEngineConfig {
...
fn native_execution_descriptor_round_trips_external_provider_config() {
    let config = engine_config(NativeTimingModelConfig::External {
    ...
    assert_eq!(
        serde_json::from_value::<NativeReplayEngineConfig>(value).unwrap(),
        config
    );
}
...
fn built_in_aggregated_replay_produces_a_deterministic_report() {
    let spec = spec(engine_config(NativeTimingModelConfig::Fixed {
        prefill_ms: 10.0,
        decode_ms: 2.0,
    }));
    let first = run_native_replay(spec.clone()).unwrap();
    let second = run_native_replay(spec).unwrap();
...
fn replay_report_retains_authored_request_correlation() {
    let mut replay = spec(engine_config(NativeTimingModelConfig::Fixed {
        prefill_ms: 1.0,
        decode_ms: 1.0,
    }));
    ...
    let report = run_native_replay(replay).unwrap();
...
fn multi_worker_round_robin_uses_each_logical_worker_deterministically() {
    ...
    let first = run_native_replay(replay.clone()).unwrap();
    let second = run_native_replay(replay).unwrap();
    assert_deterministic(&first, &second);
...
fn scaling_composition_changes_round_robin_capacity_before_arrival() {
    ...
    let report = Replayer::with_composition(
        replay,
        aisimulate_replay::NativeReplayFactory::new(),
        composition,
    )
    .unwrap()
    .run()
...
#[test]
fn native_replay_preserves_exact_output_token_plan() {
    let mut replay = spec(engine_config(NativeTimingModelConfig::Fixed {
        prefill_ms: 1.0,
        decode_ms: 1.0,
    }));
...
#[test]
fn native_replay_rejects_an_out_of_range_preassigned_dp_rank() {
    let mut config = engine_config(NativeTimingModelConfig::Fixed {
        prefill_ms: 1.0,
        decode_ms: 1.0,
    });
    config.dp_size = 2;
    ...
    let error = run_native_replay(replay).unwrap_err();
...
fn impossible_disaggregated_sglang_prefill_is_not_hidden_as_an_external_wait() {
    let fixed = NativeTimingModelConfig::Fixed {
        prefill_ms: 1.0,
        decode_ms: 1.0,
    };
    let mut replay = disaggregated_spec(NativeBackend::Sglang, fixed.clone(), fixed);
    let mut config: NativeReplayEngineConfig =
...
fn resource_accounting_multiplies_attention_dp_and_tensor_parallelism() {
    ...
    let report = run_native_replay(spec(config)).unwrap();
...
impl NativeTimingModel for FixedExternalTiming {
...
fn runner_must_resolve_external_timing_before_execution() {
    let spec = spec(engine_config(NativeTimingModelConfig::External {
        provider: "aic".to_string(),
        config: serde_json::json!({"model": "test"}),
    }));
    let error = run_native_replay(spec.clone()).unwrap_err();
    assert!(
        error
            .to_string()
            .contains("timing provider 'aic' requires NativeEngineFactory::with_timing_model"),
        "{error}"
    );

    let report = run_native_replay_with_timing(
        spec,
...
fn disaggregated_handoff_latency_is_used_when_engine_timing_is_missing() {
    ...
    let mut zero = disaggregated_spec(NativeBackend::Vllm, fixed.clone(), fixed);
    ...
    let zero_report = run_native_replay(zero).unwrap();
    let fallback_report = run_native_replay(fallback).unwrap();
    assert_eq!(
...
fn disaggregated_prefill_does_not_reserve_or_generate_the_decode_output_length() {
    for backend in [NativeBackend::Vllm, NativeBackend::Sglang] {
        ...
        let report = run_native_replay(spec)
...
fn role_specific_timing_models_support_external_and_builtin_mixes() {
    let external = NativeTimingModelConfig::External {
        provider: "aic".to_string(),
    ...
    let spec = disaggregated_spec(NativeBackend::Vllm, external.clone(), external);
    let report = run_native_replay_with_optional_role_timing(
        spec,
    ...
    let spec = disaggregated_spec(
        NativeBackend::Vllm,
        NativeTimingModelConfig::External {
            provider: "aic".to_string(),
            config: serde_json::json!({"model": "prefill"}),
        },
        fixed,
...
fn native_trtllm_disaggregated_replay_is_an_explicit_error() {
    ...
    let error = run_native_replay(spec).unwrap_err();
    assert!(
        error
            .to_string()
