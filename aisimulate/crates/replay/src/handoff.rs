...
use aisimulate_engine::NativeBackend;
...
impl NormalizedHandoffConformance {
    ...
    pub fn validate(&self) -> Result<()> {
        let expected_order = match self.engine_type {
            NativeBackend::Vllm => HandoffOrder::SourceFirst,
            NativeBackend::Sglang => HandoffOrder::DestinationFirst,
            NativeBackend::Trtllm => bail!("TRT-LLM does not support destination handoff"),
        };
