...
use crate::native::{NativeReplayEngineConfig, NativeReplayFactory, NativeReplayRoleConfig};
use crate::replay::TraceSimulationReport;
use aisimulate_engine::{
    NativeBackend as EngineType, NativeEngineConfig as MockEngineArgs, NativeKvEvent,
    NativeKvEventData, NativeWorkerType as WorkerType, TransferTimingMode as KvTransferTimingMode,
};
...
impl TestDisaggConfig {
    fn runtime_config(&self, emit_kv_events: bool) -> anyhow::Result<OfflineDisaggReplayConfig> {
        let engine = NativeReplayEngineConfig {
            ...
            prefill: Some(NativeReplayRoleConfig {
                dp_size: 1,
                tensor_parallel_size: 1,
                rank: self.prefill_args.clone(),
            }),
            decode: Some(NativeReplayRoleConfig {
                dp_size: 1,
                tensor_parallel_size: 1,
...
struct NativeEventBatch(Vec<NativeKvEvent>);
...
type HandoffDisaggRuntime = DisaggRuntimeImpl<
