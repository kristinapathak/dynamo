...
use aisimulate_engine::{NativeBackend, NativeTimingModel, NativeTimingModelConfig};
use aisimulate_replay::{
    NativeReplayEngineConfig, NativeReplayRoleConfig, ReplaySpec, ReplayTopology,
    run_native_replay, run_native_replay_with_optional_role_timing, run_native_replay_with_timing,
};
...
impl NativeTimingModel for AicTimingModel {
...
fn estimate_aic_num_gpu_blocks(
    config: &AicTimingConfig,
    role: &NativeReplayRoleConfig,
) -> Result<usize> {
...
fn materialize_aic_capacity(
    config: &AicTimingConfig,
    role: &mut NativeReplayRoleConfig,
    capacity_is_explicit: bool,
    estimate: impl FnOnce(&AicTimingConfig, &NativeReplayRoleConfig) -> Result<usize>,
) -> Result<()> {
    config.validate_parallel_shape()?;
    let engine_backend = match role.rank.backend {
        NativeBackend::Vllm => "vllm",
        NativeBackend::Sglang => "sglang",
        NativeBackend::Trtllm => "trtllm",
    };
    ensure!(
...
fn aggregated_role(engine: &NativeReplayEngineConfig) -> NativeReplayRoleConfig {
...
fn resolve_role_timing(
    role: &mut NativeReplayRoleConfig,
    capacity_is_explicit: bool,
) -> Result<Option<Arc<dyn NativeTimingModel>>> {
    let NativeTimingModelConfig::External { provider, config } = role.rank.timing_model.clone()
    else {
        return Ok(None);
    };
    ensure!(
...
fn execute_json(payload: &str) -> Result<String> {
    ...
    let mut engine_config: NativeReplayEngineConfig = if spec.engine.is_null() {
    ...
    let report = match spec.topology.clone() {
        ReplayTopology::Aggregated { .. } => {
            ...
            match timing {
                Some(timing) => run_native_replay_with_timing(spec, timing),
                None => run_native_replay(spec),
            }
        }
        ReplayTopology::Disaggregated { .. } => {
            ...
            spec.engine = serde_json::to_value(&engine_config)
                .context("serializing materialized native engine descriptor")?;
            run_native_replay_with_optional_role_timing(spec, prefill_timing, decode_timing)
        }
    }
...
mod tests {
    ...
    use aisimulate_replay::{
        NativeReplayEngineConfig, ProviderSpec, ReplayAdapters, ReplayRequest, ReplaySpec,
    ...
    fn capacity_is_estimated_only_when_not_explicit() {
        let mut role = aggregated_role(&NativeReplayEngineConfig::default());
        materialize_aic_capacity(&aic_config(), &mut role, false, |_config, rank| {
            assert_eq!(
                rank.rank.block_size,
                NativeEngineConfig::default().block_size
            );
            Ok(321)
        })
        .unwrap();
