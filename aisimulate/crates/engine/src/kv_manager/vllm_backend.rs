...
impl VllmKvManager {
    ...
    fn publish_kv_event(
        ...
        let data = if is_store {
            NativeKvEventData::Stored(NativeStoredBlocks {
                parent_hash,
                start_position: None,
                blocks: full_blocks
                    .into_iter()
                    .enumerate()
                    .map(|(index, hash)| NativeKvBlock {
                        block_hash: hash,
...
mod tests {
    ...
    impl CapturingNativeSink {
        fn take(&self) -> Vec<NativeKvEvent> {
    ...
    fn event_enabled_finalization_preserves_store_payload() {
        ...
        let NativeKvEventData::Stored(stored) = event.data else {
