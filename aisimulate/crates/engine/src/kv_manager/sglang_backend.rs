...
use crate::native::{NativeKvBlock, NativeKvEvent, NativeKvEventData, NativeStoredBlocks};
...
pub struct SglangKvManager {
    ...
    page_to_block_hash: FxHashMap<KvPageId, SequenceHash>,
...
impl SglangKvManager {
    ...
    fn publish_stored_hashes(
        ...
        let event = NativeKvEvent {
            event_id: self.next_event_id,
            data: NativeKvEventData::Stored(NativeStoredBlocks {
                parent_hash,
                start_position: None,
                blocks,
            }),
    ...
    fn publish_removed_pages(&mut self, evicted_pages: &[KvPageId]) {
        ...
        let event = NativeKvEvent {
            event_id: self.next_event_id,
            data: NativeKvEventData::Removed { block_hashes },
            dp_rank: self.dp_rank,
        };
        self.next_event_id += 1;
...
mod tests {
    ...
    struct MockSink {
        events: Mutex<Vec<NativeKvEvent>>,
    }
    ...
    impl MockSink {
        ...
        fn clone_events(&self) -> Vec<NativeKvEvent> {
            self.events.lock().unwrap().clone()
        }
    }

    impl KvCacheEventSink for MockSink {
        fn publish(&self, event: NativeKvEvent) -> anyhow::Result<()> {
    ...
    fn test_event_publishing_uses_native_block_hashes() {
        ...
        let NativeKvEventData::Stored(store) = &events[0].data else {
    ...
    fn test_published_prefix_hashes_only_unseen_suffix() {
        ...
        let NativeKvEventData::Stored(first) = &events[0].data else {
            panic!("expected first stored event");
        };
        let NativeKvEventData::Stored(second) = &events[1].data else {
    ...
    fn test_cache_materialization_processes_only_newly_completed_blocks() {
        ...
        let events = sink.clone_events();
        assert_eq!(events.len(), 1);
        let NativeKvEventData::Stored(first_store) = &events[0].data else {
        ...
        let events = sink.clone_events();
        assert_eq!(events.len(), 2);
        let NativeKvEventData::Stored(final_store) = &events[1].data else {
    ...
    fn test_duplicate_logical_blocks_publish_once_and_remove_once() {
        ...
        let events = sink.clone_events();
        assert_eq!(events.len(), 2);
        let NativeKvEventData::Removed { block_hashes } = &events[1].data else {
    ...
    fn test_chunked_prefill_parent_hash() {
        ...
        let NativeKvEventData::Stored(store1) = &events[0].data else {
            panic!("expected first event to be Stored");
        };
        let NativeKvEventData::Stored(store2) = &events[1].data else {
