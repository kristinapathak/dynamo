...
//!
//! Serializable user configuration lives in `crate::NativeEngineConfig`. The
...
/// Rank-local view of `NativeEngineConfig`.
///
/// This is deliberately not serializable and performs no provider loading or
/// validation. `NativeEngineConfig::validate` is the single configuration
/// boundary; `scheduler::native_rank::core_args` materializes this view.
