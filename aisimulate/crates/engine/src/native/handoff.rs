...
//!
//! These value types are owned by `aisimulate-engine` because native
...
use super::config::NativeWorkerType;
...
///
/// Native schedulers publish this value when source KV ownership becomes held.
...
pub fn prefill_handoff_delay_ms(
    ...
    if worker_type != NativeWorkerType::Prefill || !completed {
        return None;
    }
...
mod tests {
    ...
    fn prefill_handoff_delay_requires_completed_prefill_work() {
        ...
        assert_eq!(
            prefill_handoff_delay_ms(NativeWorkerType::Prefill, true, args.0, args.1, args.2,),
            Some(128.0)
        );
        assert_eq!(
            prefill_handoff_delay_ms(NativeWorkerType::Prefill, false, args.0, args.1, args.2,),
            None
        );
        assert_eq!(
            prefill_handoff_delay_ms(NativeWorkerType::Decode, true, args.0, args.1, args.2,),
            None
        );
    }
}
