...
use crate::common::perf_model::{polynomial_decode_time, polynomial_prefill_time};

/// Serializable timing-provider selection.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case", tag = "type")]
pub enum NativeTimingModelConfig {
...
/// provider without adding that dependency to `aisimulate-engine`.
pub trait NativeTimingModel: Send + Sync {
...
impl NativeTimingModel for PolynomialTimingModel {
    fn predict_prefill_ms(
        &self,
        batch_size: usize,
        mean_isl: usize,
...
impl NativeTimingModel for FixedTimingModel {
    fn predict_prefill_ms(
        &self,
        batch_size: usize,
        _mean_isl: usize,
...
pub(crate) fn built_in_timing_model(
    config: &NativeTimingModelConfig,
) -> Result<Arc<dyn NativeTimingModel>> {
    match config {
        NativeTimingModelConfig::Polynomial => Ok(Arc::new(PolynomialTimingModel)),
        ...
        NativeTimingModelConfig::External { provider, .. } => {
            bail!("timing provider '{provider}' requires NativeEngineFactory::with_timing_model")
        }
    }
}
...
mod tests {
    ...
    fn fixed_model_returns_zero_for_empty_batches() {
        let model = built_in_timing_model(&NativeTimingModelConfig::Fixed {
            prefill_ms: 7.0,
            decode_ms: 3.0,
        })
        .unwrap();
    ...
    fn external_provider_requires_runner_resolution() {
        let error = built_in_timing_model(&NativeTimingModelConfig::External {
        ...
        assert!(
            error
                .to_string()
                .contains("NativeEngineFactory::with_timing_model")
        );
    }
}
