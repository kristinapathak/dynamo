...
use crate::native::{HandoffId, modeled_duration_ms};
#[cfg(test)]
...
impl VllmCore {
    ...
    pub(crate) fn drain_kv_events(&self) -> Vec<crate::native::NativeKvEvent> {
    ...
    pub(super) fn execute_pass_internal(
        ...
        Ok(EnginePassResult {
            ...
            kv_event_visibility: KvEventVisibility::PassStart,
            kv_events: self
                .kv_event_buffer
                .as_ref()
                .map(CapturedKvEventBuffer::drain)
    ...
    fn emit_speculative_ready_tokens(
        ...
        let mut reservation = loop {
            ...
            let Some(_preempted) = self.policy_preempt() else {
                if running_changed {
                    self.state.compact_running();
                }
