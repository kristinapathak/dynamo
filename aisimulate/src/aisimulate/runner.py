...
"""Dynamo-free native implementation of the Spica replay-runner contract."""
...
from .spica.adapter import JSONValue
from .spica.replay import ReplayReport, ReplaySpec, RunnerCapabilities
...
class InvalidRunnerError(RuntimeError):
    """An installed runtime or adapter does not implement the runner contract."""


@runtime_checkable
class NativeReplayBackend(Protocol):
    """Python surface exported by the optional native AISimulate runtime."""
...
class NativeReplayRunnerFactory:
    """Create reusable native runners for backend-only Spica candidates.

    The optional backend is a test seam. Production factories leave it unset so
    ...
    backend: NativeReplayBackend | None = field(default=None, repr=False, compare=False)
    ...
    def create(self, worker_id: int) -> "NativeReplayRunner":
        return NativeReplayRunner(
            worker_id=worker_id,
            capabilities=self.capabilities(),
            trace_block_size=self.trace_block_size,
            include_native_report=self.include_native_report,
...
class NativeReplayRunner:
    """Execute one backend-only candidate with the native AISimulate Replayer."""

    worker_id: int
    capabilities: RunnerCapabilities
    trace_block_size: int = 512
    include_native_report: bool = False
    backend: NativeReplayBackend | None = None

    def _resolve_backend(self) -> NativeReplayBackend:
        ...
        try:
            ...
            raise RunnerUnavailableError(
                "AISimulate native replay runtime is not installed. Install a "
                "binary aisimulate package or build its native extension."
            ) from exc
        except ImportError as exc:
            raise RunnerUnavailableError(
                "AISimulate native replay runtime could not be loaded. Reinstall "
                "the binary aisimulate package or rebuild its native extension."
            ) from exc

        if not isinstance(runtime, NativeReplayBackend):
    ...
    def run(self, spec: ReplaySpec) -> ReplayReport:
        ...
        execution_spec = _materialize_engine_execution_spec(
            spec,
            trace_block_size=self.trace_block_size,
            record_per_request=self.include_native_report,
        )
        ...
        report_json = self._resolve_backend().run_replay_json(execution_spec_json)
        if not isinstance(report_json, str):
            raise InvalidRunnerError(
                "AISimulate native runtime report must be a JSON string"
            )
        try:
            report = json.loads(report_json)
        except json.JSONDecodeError as exc:
            raise InvalidRunnerError(
                "AISimulate native runtime returned invalid report JSON"
            ) from exc
        if not isinstance(report, Mapping):
            raise InvalidRunnerError(
                "AISimulate native runtime report must be a JSON object"
            )
        return _native_replay_report(
            report, include_native_report=self.include_native_report
        )
...
def _materialize_engine_execution_spec(
    ...
    *,
    ...
    This is the only high-level-to-execution materializer. The native binding
    deliberately knows nothing about CLI, Spica, or provider discovery.
    """
    ...
    if deployment_mode == "agg":
        ...
        engine = _materialize_native_role(
            deployment.backend,
        ...
        if deployment.backend == "trtllm":
            raise ValueError("native TensorRT-LLM replay does not support disagg")
        ...
        prefill = _materialize_native_role(
            deployment.backend,
            deployment.backend_version,
            deployment.parallel_config,
            raw_prefill,
        ...
        decode = _materialize_native_role(
            deployment.backend,
            deployment.backend_version,
            deployment.parallel_config,
            raw_decode,
        ...
        if prefill_backend != decode_backend:
            raise ValueError(
                "disaggregated prefill and decode must use the same native backend: "
                f"{prefill_backend!r} != {decode_backend!r}"
            )
        if prefill_backend == "trtllm":
            raise ValueError(
                "native TensorRT-LLM replay does not support disaggregated mode"
            )
        ...
        raise ValueError(
            "native replay deployment_mode must be 'agg' or 'disagg', got "
            f"{deployment_mode!r}"
        )
...
def _materialize_native_role(
    ...
    if backend not in {"vllm", "sglang", "trtllm"}:
        raise ValueError(
            f"native engine replay supports vllm, sglang, and trtllm, got {backend!r}"
        )
...
def _require_supported_synthetic_workload(
    ...
    if unsupported:
        raise ValueError(
            "native engine synthetic replay does not yet support "
            + ", ".join(unsupported)
        )
...
def _native_replay_report(
    ...
    """Flatten the native report into Spica's stable scoring metric names."""
    ...
    def add(name: str, value: object) -> None:
        ...
        if not math.isfinite(number):
            raise InvalidRunnerError(f"native replay metric {name!r} is not finite")
        metrics[name] = number
