...
"""CPU Mocker integration coverage for weighted Router policy classes."""
...
async def _send_balanced_workload(frontend_port: int) -> ClientResult:
    ...
    ready = asyncio.Barrier(2 * REQUESTS_PER_CLASS + 1)
    ...
    async with aiohttp.ClientSession(
        ...
        # Participate in the barrier so neither class can send before every
        # class-balanced client task is ready.
        await ready.wait()
        outcomes = []
