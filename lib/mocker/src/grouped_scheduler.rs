...
#[cfg(test)]
use aisimulate_engine::NativeKvEventData;
use aisimulate_engine::{
    EngineEffects, EngineIdentity, EnginePassCompleted, GeneralizedMockerEngine, NativeAdmission,
    NativeCommand, NativeCommandEffects, NativeCommandResult, NativeEngineFactory,
    NativeForwardPassMetrics, NativeKvEvent, NativeMetrics, NativePassCompletionEffects,
    RankIdentity, SchedulerCommand as NativeSchedulerCommand,
};
...
use crate::native_config::{NativeEngineComponents, native_engine_components};
use crate::native_observations::{dynamo_forward_pass_snapshot, dynamo_kv_event};
use crate::scheduler::{
...
pub(crate) fn create_grouped_scheduler_with_event_senders(
    ...
    let components = native_engine_components(args, emit_kv_events, emit_kv_token_ids)?;
...
fn create_grouped_scheduler_from_components(
    components: NativeEngineComponents,
    rank_sinks: Vec<GroupedSchedulerRankEventSinks>,
    ...
    let factory = NativeEngineFactory::with_timing_model(components.rank, components.timing)?;
...
async fn forward_command(
    ...
    let queued = match bridge
        ...
        .enqueue_reserved_command(
            command_id,
            NativeSchedulerCommand::new(bridge.dp_rank, translated.command),
        )
        .await
    {
        Ok(queued) => queued,
...
fn translate_command(
    ...
    let (command, on_success, on_suppressed_output, on_error) = match command {
        ...
        SchedulerCommand::SubmitHandoffPrefill {
            ...
            let native_handoff = compatibility.native_handoff(handoff_id)?;
            let request = compatibility.native_request(request);
            let request_id = request.request_id;
            compatibility.mark_source(handoff_id, request_id);
            (
                NativeCommand::SubmitHandoffPrefill {
                    handoff_id: native_handoff,
                    request,
                },
                Vec::new(),
                Vec::new(),
        ...
        SchedulerCommand::ReleaseSource { handoff_id } => (
            NativeCommand::ReleaseSource {
                handoff_id: compatibility.native_handoff(handoff_id)?,
            },
        ...
        SchedulerCommand::ActivateDestination { handoff_id } => (
            NativeCommand::ActivateDestination {
                handoff_id: compatibility.native_handoff(handoff_id)?,
            },
            Vec::new(),
            Vec::new(),
        ...
        SchedulerCommand::CancelDestination { handoff_id } => (
            NativeCommand::CancelDestination {
                handoff_id: compatibility.native_handoff(handoff_id)?,
            },
