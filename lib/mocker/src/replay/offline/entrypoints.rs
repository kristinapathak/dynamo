...
use aisimulate_replay::{
    CURRENT_REPLAY_SPEC_VERSION, NativeReplayEngineConfig, ProviderSpec, ReplayAdapters,
    ReplayRuntimeInput, ReplayScalingPolicy, ReplaySpec, ReplayTopology, Replayer, WorkerPoolSpec,
};
use anyhow::Result;
...
use crate::native_config::{native_aggregated_replay_setup, native_disaggregated_replay_setup};
use crate::replay::{
...
fn replay_spec(
    ...
    engine: NativeReplayEngineConfig,
...
fn run_aggregated(
    ...
    let (engine, factory) = native_aggregated_replay_setup(&args)?;
...
fn run_disaggregated(
    ...
    let (engine, factory) =
        native_disaggregated_replay_setup(&config.prefill_args, &config.decode_args)?;
    let spec = replay_spec(
...
pub fn run_offline_handoff_conformance(
    ...
    let (engine, factory) = native_disaggregated_replay_setup(&prefill_args, &decode_args)?;
    ...
    Ok(aisimulate_replay::run_native_handoff_conformance(engine, factory, request)?.into())
}
