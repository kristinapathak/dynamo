...
use aisimulate_engine::{
    EngineEffects, EnginePassCompleted, EnginePassStarted, GeneralizedMockerEngine, NativeCommand,
    NativeCommandEffects, NativePassCompletionEffects, NativePassStartEffects, NativeRankEngine,
    SchedulerCommand,
};
...
pub type NativeGroupedEngine = GeneralizedMockerEngine<NativeRankEngine>;
...
pub enum GroupedLiveEvent {
    ...
    PassStarted(EnginePassStarted<NativePassStartEffects>),
...
struct ControlEnvelope {
    command_id: u64,
    command: SchedulerCommand<NativeCommand>,
...
enum BoundaryRequest {
    Apply {
        command: SchedulerCommand<NativeCommand>,
        reply: oneshot::Sender<Result<EngineEffects<NativeCommandEffects>>>,
    },
    Finish,
}
...
impl GroupedPassBoundary {
    pub(crate) async fn apply_command(
        &self,
        command: SchedulerCommand<NativeCommand>,
    ) -> Result<EngineEffects<NativeCommandEffects>> {
...
impl GroupedLiveEngineHandle {
    ...
    ///
    /// [`NativeCommand::CancelRequest`] is automatically routed through the
    /// dedicated cancellation lane. Other commands retain ordinary FIFO
    /// ordering.
    #[cfg(test)]
    async fn apply_command(&self, command: SchedulerCommand<NativeCommand>) -> Result<()> {
    ...
    pub(crate) async fn enqueue_reserved_command(
        ...
        let is_cancellation = matches!(command.command, NativeCommand::CancelRequest { .. });
...
/// Drive one single-rank or attention-DP native engine using Tokio's wall
/// clock.
///
...
fn command_can_apply_during_pass(command: &NativeCommand) -> bool {
    matches!(
        command,
        NativeCommand::SubmitHandoffPrefill { .. } | NativeCommand::ReserveDestination { .. }
    )
}
...
mod tests {
    ...
    use aisimulate_engine::{
        EngineIdentity, HandoffId, NativeCommandResult, NativeEngineConfig, NativeEngineFactory,
        NativeRequest, NativeTimingModelConfig, NativeWorkerType,
    };

    use super::*;

    fn request(id: u128, prompt_len: usize, output_len: usize) -> NativeRequest {
        NativeRequest {
    ...
    fn runtime_with_config(dp_size: u32, config: NativeEngineConfig) -> GroupedLiveRuntime {
        let engine = NativeEngineFactory::new(config)
            .unwrap()
    ...
    async fn attention_dp_releases_completion_only_at_the_shared_boundary() {
        ...
        let rank0 = handle.apply_command(SchedulerCommand::new(
            0,
            NativeCommand::Submit(request(1, 4, 1)),
        ));
        let rank1 = handle.apply_command(SchedulerCommand::new(
            1,
            NativeCommand::Submit(request(2, 8, 1)),
        ));
    ...
    async fn cancellation_suppresses_retained_output_during_a_grouped_pass() {
        ...
        handle
            .apply_command(SchedulerCommand::new(
                0,
                NativeCommand::Submit(request(11, 4, 1)),
            ))
            .await
            .unwrap();
        assert!(matches!(
    ...
    async fn queued_cancellation_preempts_an_overdue_pass_boundary() {
        let mut engine = NativeEngineFactory::new(NativeEngineConfig {
            num_gpu_blocks: 128,
            block_size: 4,
            max_num_seqs: 8,
            max_num_batched_tokens: 256,
        ...
        engine
            .apply_command_effects(
                SchedulerCommand::new(0, NativeCommand::Submit(request(12, 4, 2))),
                0.0,
            )
            .unwrap();
    ...
    async fn noop_cancellation_outcome(discard_pending_output: bool) -> (bool, usize) {
        ...
        handle
            .apply_command(SchedulerCommand::new(
                0,
                NativeCommand::Submit(request(21, 4, 1)),
            ))
            .await
            .unwrap();
        assert!(matches!(
        ...
        assert_eq!(effects.by_rank[0].effects.result, NativeCommandResult::Noop);
    ...
    async fn held_source_waits_for_release_without_empty_passes() {
        let GroupedLiveRuntime {
            ...
            1,
            NativeEngineConfig {
                worker_type: NativeWorkerType::Prefill,
                num_gpu_blocks: 128,
                block_size: 4,
                max_num_seqs: 8,
                max_num_batched_tokens: 256,
        ...
        assert!(matches!(
            ...
            [aisimulate_engine::NativeLifecycleEvent::SourceHeld { handoff_id: observed, .. }]
    ...
    async fn midpass_idle_sibling_completes_with_group_metrics() {
        ...
        handle
            .apply_command(SchedulerCommand::new(
                0,
                NativeCommand::Submit(request(71, 4, 1)),
            ))
            .await
            .unwrap();
        assert!(matches!(
    ...
    async fn cancellation_does_not_end_a_pass_with_unrelated_pending_output() {
        ...
        handle
            .apply_command(SchedulerCommand::new(
                0,
                NativeCommand::Submit(request(31, 4, 1)),
            ))
            .await
            .unwrap();
        assert!(matches!(
        ...
        handle
            ...
            .unwrap();
        let GroupedLiveEvent::CommandApplied { effects, .. } = next_event(&mut events).await else {
            panic!("expected unrelated cancellation effects");
        };
        assert_eq!(effects.by_rank[0].effects.result, NativeCommandResult::Noop);
        assert!(!effects.by_rank[0].effects.suppressed_pending_output);
    ...
    async fn external_shutdown_stops_a_nonempty_zero_duration_progress_loop() {
        ...
        handle
            .apply_command(SchedulerCommand::new(
                0,
                NativeCommand::Submit(request(41, 4, 32)),
            ))
            .await
            .unwrap();
        assert!(matches!(
