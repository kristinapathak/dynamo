...
use aisimulate_engine::{NativeForwardPassMetrics, NativeKvEvent, NativeKvEventData};
...
pub(crate) fn dynamo_kv_event(event: NativeKvEvent) -> (KvCacheEvent, Option<Vec<Vec<u32>>>) {
    let (data, block_token_ids) = match event.data {
        NativeKvEventData::Stored(stored) => {
        ...
        NativeKvEventData::Removed { block_hashes } => (
...
pub(crate) fn dynamo_forward_pass_snapshot(
    dp_rank: u32,
    metrics: NativeForwardPassMetrics,
) -> ForwardPassSnapshot {
    ForwardPassSnapshot {
        dp_rank,
...
mod tests {
    ...
    use aisimulate_engine::{NativeKvBlock, NativeStoredBlocks};
    ...
    fn stored_event(
        ...
        NativeKvEvent {
            ...
            data: NativeKvEventData::Stored(NativeStoredBlocks {
                ...
                blocks: blocks
                    .iter()
                    .map(|(block_hash, tokens_hash, token_ids)| NativeKvBlock {
    ...
    fn stored_and_removed_events_preserve_router_identity_and_token_metadata() {
        ...
        let (event, token_ids) = dynamo_kv_event(NativeKvEvent {
            event_id: 18,
            dp_rank: 3,
    ...
    async fn converted_store_and_remove_events_apply_cleanly_to_the_dynamo_indexer() {
        ...
        let removed = dynamo_kv_event(NativeKvEvent {
            event_id: 3,
            dp_rank: 3,
            data: NativeKvEventData::Removed {
                block_hashes: vec![101, 102],
