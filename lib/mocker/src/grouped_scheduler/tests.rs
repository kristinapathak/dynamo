...
async fn noop_cancellation_only_cleans_metadata_when_output_is_discarded() {
    for (suppressed_pending_output, expect_handoff_delay) in [(false, true), (true, false)] {
        ...
        dispatch_command_effects(
            23,
            EngineEffects {
                by_rank: vec![aisimulate_engine::RankEffects {
                    dp_rank: 0,
                    effects: NativeCommandEffects {
                        result: NativeCommandResult::Noop,
        ...
        let output = compatibility.output_signal(aisimulate_engine::NativeOutput {
            request_id,
            token_id: Some(9),
            completed: true,
            rejected: false,
...
#[tokio::test]
async fn midpass_cancel_ack_precedes_completion_metrics() {
    let effects = Arc::new(CapturedEffects::default());
    let mut slow_args = args(1);
    ...
    assert_eq!(
        metrics.borrow().active_decode_blocks,
        before_cancel.active_decode_blocks,
        "mid-pass command metrics must remain hidden until pass completion"
    );
    ...
    .await
    .expect("pass completion should publish the post-cancel metrics");
    assert_eq!(
        effects.publication_log.lock().unwrap().as_slice(),
...
async fn synthetic_midpass_kv_is_deferred_until_completion_before_fpm() {
    ...
    let command_effects = EngineEffects {
        by_rank: vec![aisimulate_engine::RankEffects {
            dp_rank: 0,
            effects: NativeCommandEffects {
                result: NativeCommandResult::Applied,
                lifecycle_events: Vec::new(),
                kv_events: vec![NativeKvEvent {
                    event_id: 1,
                    dp_rank: 0,
                    data: NativeKvEventData::Removed {
                        block_hashes: vec![42],
                    },
                }],
                retired_requests: Vec::new(),
    ...
    publish_pass_router_effects(
        ...
        Vec::new(),
        &mut deferred[0].kv,
        NativeForwardPassMetrics::default(),
    );
...
fn completion_metrics_override_the_midpass_command_snapshot() {
    let mut deferred = Some(NativeMetrics {
        dp_rank: 1,
        active_blocks: 99,
        running_requests: 7,
        ..NativeMetrics::default()
    });
    let completed = NativeMetrics {
        dp_rank: 1,
        active_blocks: 3,
        running_requests: 1,
        ..NativeMetrics::default()
